...
#		self.front_left_leg = mmcAngularLegModel(self.motivationNetRobot.front_left_leg)
#		self.front_right_leg = mmcAngularLegModel(self.wRobot.front_right_leg)
#		self.middle_left_leg = mmcAngularLegModel(self.wRobot.middle_left_leg)
#		self.middle_right_leg = mmcAngularLegModel(self.wRobot.middle_right_leg)
#		self.hind_left_leg  = mmcAngularLegModel(self.wRobot.hind_left_leg)
#		self.hind_right_leg  = mmcAngularLegModel(self.wRobot.hind_right_leg)
		
		#REPLACE INIT BY REAL ROBOT VALUES
	...
	def getCurrentGlobalBodyPosition(self):
		...
		foot_on_ground = 0
		while not(self.gc[-1][foot_on_ground]) and not(foot_on_ground>4):
			foot_on_ground += 1
		segm1_vect = self.foot_global[foot_on_ground] - self.front_vect[foot_on_ground][-1]
		if foot_on_ground > 1:
			segm1_vect += self.segm[0][-1]
		if foot_on_ground > 3:
			segm1_vect += self.segm[1][-1]
		return (segm1_vect)
	...
	def turnOnCurrentBodyModelDrawing(self):
		...
		self.initial_robot_translation = numpy.array([self.imu.position[0]+0.71, self.imu.position[1],self.height])
	...
	def sendCurrentBodyModelDrawing(self):
		if (self.bodyModelDrawing):
			relative_rotation = self.imu.rotation[2] - math.atan2(self.segm[0][0][1], self.segm[0][0][0])
			...
			relative_translation = self.initial_robot_translation + (self.getCurrentGlobalBodyPosition() - self.initial_relative_translation)
			#print("Rel TL: ", relative_translation)
			current_rel_transl = relative_translation + self.rotate_vector_around_z(relative_rotation, (- 0.8 * self.segm[0][-1]))
		
			# Add front legs to visualization
			leg_points = self.front_right_leg.getLegPoints()
			for i in range(0,4):
			...
			leg_points = self.front_left_leg.getLegPoints()
			for i in range(0,4):
				leg_points[i] = self.rotate_vector_around_z(relative_rotation, leg_points[i])
			for i in range (0,3):
			...
			current_rel_transl = current_rel_transl + self.rotate_vector_around_z(relative_rotation, (- 0.2 * self.segm[0][-1] - 0.8 * self.segm[1][-1]))
			leg_points = self.middle_right_leg.getLegPoints()
			for i in range(0,4):
				leg_points[i] = self.rotate_vector_around_z(relative_rotation, leg_points[i])
			for i in range (0,3):
				self.appendPointList((current_rel_transl + leg_points[i]), (current_rel_transl + leg_points[i+1]), points)
			leg_points = self.middle_left_leg.getLegPoints()
			for i in range(0,4):
				leg_points[i] = self.rotate_vector_around_z(relative_rotation, leg_points[i])
			for i in range (0,3):
				self.appendPointList((current_rel_transl + leg_points[i]), (current_rel_transl + leg_points[i+1]), points)
			...
			current_rel_transl = current_rel_transl + self.rotate_vector_around_z(relative_rotation, (- 0.2 * self.segm[1][-1] - 0.3 * self.segm[2][-1]))
			leg_points = self.hind_right_leg.getLegPoints()
			for i in range(0,4):
				leg_points[i] = self.rotate_vector_around_z(relative_rotation, leg_points[i])
			for i in range (0,3):
				self.appendPointList((current_rel_transl + leg_points[i]), (current_rel_transl + leg_points[i+1]), points)
			leg_points = self.hind_left_leg.getLegPoints()
			for i in range(0,4):
				leg_points[i] = self.rotate_vector_around_z(relative_rotation, leg_points[i])
			for i in range (0,3):
				self.appendPointList((current_rel_transl + leg_points[i]), (current_rel_transl + leg_points[i+1]), points)
			...
			self.appendPointList(relative_translation, (relative_translation + self.rotate_vector_around_z(relative_rotation, \
				(-self.segm[0][-1] - self.segm[1][-1] - self.segm[2][-1]) ) ), points)
	...
	def init_leg_mmc_models(self):
		for mmc_leg in self.mmcLegs:
			...
			#print("MMC LEG: ", mmc_leg.getInputPosition())
			#print("WLEG   : ", mmc_leg.motivationNetLeg.wleg.leg.getInputPosition() )
	...
	def updateLegStates(self):
		...
		if (self.Phase.MU_Sim.output_value > 0.) or (self.Phase.MU_Beh.output_value > 0.):
			for motiv_leg, leg_nr in zip(self.motivationNetRobot.motivationNetLegs, range(len(self.motivationNetRobot.motivationNetLegs))):
				if (motiv_leg.inSwingPhase()):
					...
					if not(self.old_stance_motivation[leg_nr]):
						self.put_leg_on_ground(motiv_leg.wleg.leg.name, \
						self.transform_robot_target_to_leg_vector( \
							self.mmcLegs[leg_nr].getFootPosition(), motiv_leg.wleg.leg.name) )
					else:
	...
	def updateLegVectorsFromLegModel(self, leg_name):
		leg_nr = RSTATIC.leg_names.index(leg_name)
		leg_vec = self.transform_robot_target_to_leg_vector( \
			self.mmcLegs[leg_nr].getFootPosition(), leg_name)
		i = 0
		# Set leg and diag vector
		self.front_vect[leg_nr][-1] = numpy.array(leg_vec)
		self.rear_vect[leg_nr][-1] = self.segm[leg_nr//2][-1] + leg_vec
...
#			self.footdiag[i][leg_nr] = self.set_up_foot_diag(i, leg_nr, -1)
	...
	def compute_leg_computations_and_integrate(self, leg_nr, step):
		if self.gc[step][leg_nr]:
			...
			return numpy.array(self.front_vect[leg_nr][step])
	...
	def compute_diag_computations_and_integrate(self, leg_nr, step):
		if self.gc[step][leg_nr]:
			...
			return numpy.array(self.rear_vect[leg_nr][step])
	...
	def mmc_iteration_step(self):
		...
		if (self.gc[-1][0]):
			...
			new_delta_front_1 = (self.rear_vect[1][-1] - self.front_vect[1][-1] - self.segm[0][-1])
		else:
			new_delta_front_1 = self.delta_front[1][-1]
		if (self.gc[-1][2]):
			new_delta_front_2 = (self.rear_vect[2][-1] - self.front_vect[2][-1] - self.segm[1][-1])
		elif (self.gc[-1][3]):
			new_delta_front_2 = (self.rear_vect[3][-1] - self.front_vect[3][-1] - self.segm[1][-1])
		else:
			new_delta_front_2 = self.delta_front[2][-1]
		...
		if (self.gc[-1][4]):
			...
			new_delta_back_1 = -(self.rear_vect[5][-1] - self.front_vect[5][-1] - self.segm[2][-1])
		else:
			new_delta_back_1 = self.delta_back[1][-1]
		if (self.gc[-1][2]):
			new_delta_back_0 = -(self.rear_vect[2][-1] - self.front_vect[2][-1] - self.segm[1][-1])
		elif (self.gc[-1][3]):
			new_delta_back_0 = -(self.rear_vect[3][-1] - self.front_vect[3][-1] - self.segm[1][-1])
		else:
			new_delta_back_0 = self.delta_back[0][-1]
		...
		# Draw plot figure of the internal model
		self.step += 1
	...
	def sendAllAngleVelocity(self):
		...
		if (self.Phase.MU_Sim.output_value > 0.5):
			if (self.simulation):
				...
				print("RESET SWING MOVEMENTs")
				#print("MMC Intern FL: ", self.front_left_leg.input_foot_position, self.front_left_leg.getInputPosition() )
				target_fr_vect = self.motivationNetRobot.bodyModelStance.get_robot_target_from_leg_vector("front_left_leg")
				#print("MMC Stance FL: ", target_fr_vect, self.wRobot.front_left_leg.leg.computeInverseKinematics(target_fr_vect) )
				# Todo: Abgleich mit realem Roboter
...
#			print("INSTABLE")
			...
			left_foot_cog_vect = -self.front_vect[left_leg][-1]
			if (left_leg == 2):
				left_foot_cog_vect = -self.front_vect[left_leg][-1] - self.segm[1][-1]
			if (left_leg == 0):
				left_foot_cog_vect = -self.front_vect[left_leg][-1] - self.segm[1][-1] - self.segm[0][-1]
