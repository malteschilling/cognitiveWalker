...
#       self.tarsus_pressure_sensor = communication_interface.CreateBfbClient(bfb_client_ids[3], ["PRESSURE_SENSOR_PROT"])
    ...
    def lock(self):
        if not self._locked:
            print('Leg',self.name,'got locked.')
            self._locked = True
            self._locked_angles = self.computeInverseKinematics(self.input_foot_position)

    def unlock(self):
        print('Leg',self.name,'got unlocked.')
        self._locked = False
        self._locked_angles = None
...
#       self.tarsus_pressure_sensor.UpdateValue('highestPressure')
    ...
    def setAngleVelocity(self, velocities):
        ...
        if self._locked:
            #print('setting angle velocities for leg', self.name)
            for joint, desired_angle, actual_angle in zip(self.joints, self._locked_angles, self.getInputAngles()):
                joint.desiredValue_ISC = float(desired_angle - actual_angle)*RSTATIC.controller_frequency
        else:
    ...
    def output_foot_position(self):
        if isinstance(self._output_foot_position, (type(None))):
    ...
    def computeInverseKinematics(self, p=None):
       ...
       # if (self.name=='middle_right_leg'):
        ...
        try:
            ...
            raise ValueError('The provided position ('+str(p_temp[0])+', '+str(p_temp[1])+', '+str(p_temp[2])+') is not valid for the given geometry for leg '+ self.name+'.'+str(self.last_pos))
        if p[2] < 0:
            beta_angle = (h1 + h2) - pi
        else:
            beta_angle = (pi - h2) + h1
    ...
    def estimateForceFromTorsion(self, torsions=None, angles=None):
        if isinstance(torsions, (type(None))):
